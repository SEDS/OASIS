// $Id: subscriber.cpp 2099 2012-01-09 22:28:58Z dfeiock $
...
#include "Processor_Data_Observer.h"
...
int main (int argc, char * argv [])
{
  try
  {
    ...
    // Register a subscriber for the ProcessorProbe data!
    ACE_ERROR ((LM_DEBUG,
    ...
    PlatformProcessorProbe::SubscriptionPtr proc_subscription (new PlatformProcessorProbe::Subscription ());
    OASIS::Test::Processor_Data_Observer * proc_data_observer = new OASIS::Test::Processor_Data_Observer (10);
    PlatformProcessorProbe::AbstractObserverPtr proc_observer (proc_data_observer);
    proc_subscription->addObserver (proc_observer);

    PlatformProcessorProbe::subscribe (*session, proc_subscription, false);

    // Register a subscriber for the MemoryProbe data!
    PlatformMemoryProbe::SubscriptionPtr mem_subscription (new PlatformMemoryProbe::Subscription ());
    OASIS::Test::Memory_Data_Observer * mem_data_observer = new OASIS::Test::Memory_Data_Observer (10);
    PlatformMemoryProbe::AbstractObserverPtr mem_observer (mem_data_observer);
    mem_subscription->addObserver (mem_observer);

    PlatformMemoryProbe::subscribe (*session, mem_subscription, false);
    ...
    proc_data_observer->wait ();
    mem_data_observer->wait ();

    // Unsubscribe from the data.
    ACE_ERROR ((LM_INFO,
                ACE_TEXT ("%T (%t) - %M - subscriber: shutting down test\n")));
    PlatformProcessorProbe::unsubscribe (*session);
