...
$the_Client_App = new PerlACE::Process ("OASIS_Daemon_Tena_Pub_Sub_Test_Subscriber", $the_Client_App_options);
...
$the_DAC_options = "-ORBEndpoint iiop://localhost:20000 --name=test --config=Daemon_Test.dac --database=test.db -v --debug";
...
$the_App_options = "--debug --hertz=5 --config=Daemon_Test.einode";
$the_App = new PerlACE::Process ("$OASIS_ROOT/bin/oasis_mon", $the_App_options);
$the_App->Spawn ();
