...
bool servant_test (::OASIS::Standard_EINode & einode)
{
  try
  {
    ...
    ::OASIS::System::Memory::MemoryProbe_Impl mem_impl;
    mem_impl.init ("--name=MemoryProbeImpl");

    if (0 != einode.register_probe (&mem_impl))
      ACE_ERROR_RETURN ((LM_ERROR,
                         ACE_TEXT ("%T (%t) - %M - failed to register mem_impl")),
                         false);
    ...
    {
      // Get the data
      if (0 != mem_impl.handle_collect_data ())
        ACE_ERROR_RETURN ((LM_ERROR,
