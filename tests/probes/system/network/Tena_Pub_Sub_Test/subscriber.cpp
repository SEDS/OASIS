// $Id: subscriber.cpp 2007 2011-10-11 12:54:21Z dfeiock $
...
int main (int argc, char * argv [])
{
  try
  {
    ...
    // Register a subscriber for the NetworkProbe data!
    ACE_ERROR ((LM_DEBUG,
    ...
    PlatformNetworkProbe::SubscriptionPtr subscription (new PlatformNetworkProbe::Subscription ());
    OASIS::Test::Data_Observer * data_observer = new OASIS::Test::Data_Observer (10);
    PlatformNetworkProbe::AbstractObserverPtr observer (data_observer);
    subscription->addObserver (observer);

    PlatformNetworkProbe::subscribe (*session, subscription, false);
    ...
    PlatformNetworkProbe::unsubscribe (*session);
