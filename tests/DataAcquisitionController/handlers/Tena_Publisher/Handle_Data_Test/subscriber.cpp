// $Id: subscriber.cpp 1957 2011-09-30 19:55:08Z hillj $
...
int main (int argc, char * argv [])
{
  try
  {
    ...
    // Register a subscriber for the Foo data!
    ACE_ERROR ((LM_DEBUG,
                ACE_TEXT ("%T (%t) - %M - subscriber: subscribing to data\n"),
                session_name.c_str ()));

    OASIS::Foo::SubscriptionPtr subscription (new OASIS::Foo::Subscription ());
    OASIS::Test::Data_Observer * data_observer = new OASIS::Test::Data_Observer (10);
    OASIS::Foo::AbstractObserverPtr observer (data_observer);
    subscription->addObserver (observer);

    OASIS::Foo::subscribe (*session, subscription, false);
