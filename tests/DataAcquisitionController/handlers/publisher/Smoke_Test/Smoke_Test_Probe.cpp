...
namespace SmokeTest
{
  ...
  {
    return ::SmokeTest::B::__metadata__;
  }
  ...
  recall (const char * data, size_t length, int byte_order) 
  {
    ::OASIS::Software_Probe_Data_Preparer prep (data, length);
    ACE_InputCDR input (prep.message_block (), byte_order);
    return ::SmokeTest::B::recall (input);
  }
  ...
  {
    return ::SmokeTest::C::__metadata__;
  }
  ...
  recall (const char * data, size_t length, int byte_order) 
  {
    ::OASIS::Software_Probe_Data_Preparer prep (data, length);
    ACE_InputCDR input (prep.message_block (), byte_order);
    return ::SmokeTest::C::recall (input);
  }
  ...
  {
    if (0 != ::SmokeTest::B::recall (in))
      ACE_ERROR_RETURN ((LM_ERROR,
                         ACE_TEXT ("%T (%t) - %M - failed to recall ")
                         ACE_TEXT ("::SmokeTest::B data\n")),
                        -1);
